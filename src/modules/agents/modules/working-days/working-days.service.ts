...
import { WorkingDaysService } from 'src/modules/agencies/modules/working-days/working-days.service';

@Injectable()
export class AgentWorkingDaysService implements CRUD<WorkingDays> {
    constructor(
        @InjectRepository(WorkingDays)
        private readonly workingDaysRepository: Repository<WorkingDays>,
        @Inject(forwardRef(() => AgentsService))
        private readonly agentsService: AgentsService,
        private readonly agencyWorkingDaysService: WorkingDaysService,
    ...
    async create(
        ...
        const existingWorkingDay = await this.workingDaysRepository.findOne({
            where: { day: createWorkingDayDto.day, agentId, isDeleted: false },
            relations: ['timeSlots'],
        });

        // If the working day exists, trough an error
    ...
    async getAll(agentId: number): Promise<WorkingDays[]> {
        // Fetch existing working days for the agent
    ...
    async getById(id: number, agentId: number): Promise<WorkingDays> {
    ...
    // Function that returns the max startTime and min endTime of all the working days of an agent
    async getMinMaxTimeHours(agentId: number): Promise<any> {
        ...
        // if there is at least one day with isOffday = false and no time slots, return 0 and 24
        ...
        // Loop through all working days and their time slots
        for (const workingDay of workingDays) {
            // If it's an off day, skip it
    ...
    async updateTimeSlot(
        ...
        await this.getById(workingDayId, agentId);
    ...
    async delete(id: number, agentId: number): Promise<void> {
        const workingDay = await this.getById(id, agentId);
    ...
    async deleteTimeSlot(
        ...
        if (workingDay.timeSlots.length === 1) {
            await this.delete(workingDayId, agentId);
            return;
        } else {
    ...
    // Function that returns an array in the format: { day: [hours] } with the availability of an agent
    async getAvailability(agentId: number): Promise<any> {
        ...
        // Get working days and their time slots for the agent
        const agentWorkingDays = await this.workingDaysRepository.find({
            where: { agentId, isDeleted: false },
            relations: ['timeSlots'],
        });
        const availabilityByDay = {};

        // Loop through the agent's working days
        for (const workingDay of agentWorkingDays) {
            // Initialize the day in the availability object if not present
            if (!availabilityByDay[workingDay.day]) {
                availabilityByDay[workingDay.day] = [];
            }

            // If it's an off day, mark the day as unavailable
            if (workingDay.isOffDay) {
                availabilityByDay[workingDay.day] = [{ isOffDay: true }];
                continue;
            }

            // Add the agent's available time slots for that day
            for (const timeSlot of workingDay.timeSlots) {
                availabilityByDay[workingDay.day].push({
        ...
        // Get the agency's working days if the agent has no custom working hours for a specific day
        const agencyWorkingDays = await this.agencyWorkingDaysService.getAll(
            agent.agencyId,
        );

        // Fill in missing days with the agency's default working hours
        for (let i = 1; i <= 7; i++) {
            // If the agent doesn't have custom working hours for a day, use the agency's working hours
            if (
                !availabilityByDay[i] ||
                (availabilityByDay[i].length === 0 &&
                    !availabilityByDay[i].isOffDay)
            ) {
                const agencyWorkingDay = agencyWorkingDays.find(
                    (day) => day.day === i,
                );

                if (agencyWorkingDay) {
                    // If the agency has a working day for this day, use the agency's time slots
                    const agencyTimeSlots = agencyWorkingDay.timeSlots.map(
                        (timeSlot) => ({
                            startTime: timeSlot.startTime,
                            endTime: timeSlot.endTime,
                        }),
                    ...
                    if (agencyTimeSlots.length > 0) {
                        availabilityByDay[i] = agencyTimeSlots;
                    } else {
                        // If no time slots exist, assume the entire day is available (0 to 24)
                        availabilityByDay[i] = [{ startTime: 0, endTime: 24 }];
                    }

                    // If the agency marks this day as an off day, mark it as unavailable for the agent
                    if (agencyWorkingDay.isOffDay) {
                        availabilityByDay[i] = [];
                    }
                } else {
                    // If no working day exists for this day, assume the entire day is available (0 to 24)
                    availabilityByDay[i] = [{ startTime: 0, endTime: 24 }];
                }
            }
        }
